    ...
    <div class="musiclist" ref="music">
      <div>
        <div class="content-box">
          <div class="cont-img">
            <div class="cover"></div>
            <img v-lazy="MusicList.coverImgUrl" />
            <p>{{this.$store.state.musicList.musicList.name}}</p>

            <!-- <p>快停止胡思乱想，夜里总要听些温柔的歌</p> -->
            <span
              class="img-cont icon-headphones"
            >&nbsp;{{Math.floor(MusicList.playCount / 10000)}}万</span>
          </div>
          <div class="content">
            <div class="play-all">
              <span class="play-all-left icon-play2"></span>
              <span class="play-all-middle">播放全部</span>
              <span class="play-all-right">(共{{MusicList.tracks.length}}首)</span>
            </div>
            <div class="play-list">
              <ul>
                <li v-for="(item,index) in MusicList.tracks" :key="index">
            ...
            <LoadingMin v-if="!this.MusicList.tracks.length" />
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  created() {
    this.getMusicListApi(this.$route.params.id);
  },
  updated() {
    this.initMusicScroll();
    // console.log("sssssssssssss", this.$store.state.musicList);
  },
  methods: {
    // 滑动功能
    initMusicScroll() {
    ...
    getMusicListApi(id) {
      ...
      axios.get(url).then(res => {
        if (res.status === 200) {
          ...
          console.error("getMusicListApi数据 获取失败");
        }
      });
    }
  }
